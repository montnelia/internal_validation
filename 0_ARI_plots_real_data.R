...
for(k_true in 2:10){
  ...
  for(tmp in c("biase.rds", "goolam.rds", "yan.rds", "treutlein.rds")){
    ...
    ss <- silhouette(res_cl_phate, D_euclidean)
    sil_phate <- mean(ss[,3])
    
    ss <- silhouette(fit_cluster_kmeans$cluster, D_euclidean) # tsne 
    sil_tsne <- mean(ss[,3])
    
    ss <- silhouette(fit_cluster_umap_kmeans$cluster, D_euclidean)
    sil_umap <- mean(ss[,3])
    
    ss <- silhouette(res_adaSC3, D_euclidean)
    sil_adaSC3 <- mean(ss[,3])
    
    ss <- silhouette(res_SC3, D_euclidean)
    sil_SC3 <- mean(ss[,3])
...
for(tmp in c("biase.rds", "deng.rds", "goolam.rds", "yan.rds", "treutlein.rds")){
  ...
  k_truth
  p<-ggplot(data_prep, aes(x=clusters, y=sil_value)) + theme(plot.title = element_text(hjust = 0.5))+
    geom_boxplot() + ggtitle("") + xlab("Number of clusters k") + ylab("Average Silhouette Scores") +
    geom_boxplot(data=data_prep[data_prep$clusters==k_truth,],
                 aes(x = clusters, y = sil_value),color="darkblue", fill="gray") 
...
for(tmp in c("biase.rds", "deng.rds", "goolam.rds", "yan.rds", "treutlein.rds")){
  ...
  k_truth <- length(levels(factor(k_true)))
  k_truth
  p<-ggplot(data_prep, aes(x=clusters, y=corr)) + theme(plot.title = element_text(hjust = 0.5))+
    geom_boxplot() + ggtitle("") + ylim(0,1) + xlab("Number of clusters k") + ylab("Correlation") +
    geom_boxplot(data=data_prep[data_prep$clusters==k_truth,],
                 aes(x = clusters, y = corr),color="darkblue", fill="gray") 
  p <- p+theme(axis.text=element_text(size=20),
               axis.title=element_text(size=22))
